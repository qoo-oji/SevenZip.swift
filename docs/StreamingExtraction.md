...
// 後方へ戻れる範囲を広げる(ブロック出力の直近 256MB を控える。次にデコーダを作るときから有効)
archive.historyByteCount = 256 << 20

// いま保持しているデコーダのメモリ(LZMA 辞書+履歴リング+バッファ、および extract のブロックキャッシュ)
...
- **履歴リング(`Archive.historyByteCount`)**: 利用側が指定したバイト数ぶん、ブロックの出力(フィルタ後)を
  リングバッファに控える。コーダの種類を問わず、その範囲なら戻れる。辞書は「最後に伸長した位置」から
  数えるので、先読みが前方へ進むほど後方の窓は狭くなる ―― ビューアが 1 ページずつ戻るときは、
  先読みが 10 ページ先まで進んでいる状態で 8 ページ前を読む、といった距離になり、16MB の辞書では
  足りない。リングはブロックの大きさ以上には確保せず、フィルタ無しの LZMA / LZMA2 で辞書が
  リング以上に大きい場合、および Copy ブロックでは確保しない(既に無料で戻れるため)。
  `residentDecoderBytes` に含まれる。

これらの範囲外へは従来どおり作り直しになります。
...
- 後方ジャンプは LZMA 辞書の範囲(通常 16〜64MB)・`historyByteCount` の履歴リング・Copy ブロック内なら
  無料、それより外はブロック先頭からのやり直し。ブロックを**前後交互**に舐めるような読み方は窓の外へ
  すぐ出るので、利用側は書庫順に読むこと(中央から前後交互に 100 エントリ 250MB を読むと、書庫順なら
  10 秒のところが 230 秒になる)。
